...
- ~~Smart Create planner honors profile preferences (Phase 2 surface)~~ — **shipped in kitesforu-api PR #250 (2026-04-20)**. Adds `build_smart_create_planning_hint(user_id)` that emits a compact directive block telling the intake LLM to use `language_preference`, `preferred_style`, `difficulty_preference`, and `preferred_duration_min` as defaults when the user's text doesn't specify them. Wired into `_build_system_prompt` as `{{profile_defaults_context}}`. A user who set Hindi + 15 min + Storytelling in `/settings/profile` now gets those defaults automatically. 4 new tests. Live on api rev 00494.
