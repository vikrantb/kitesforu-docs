...
- [x] **All 29 sub-genre entries rewritten under the same contract** — complete 2026-04-24 via frontend #576 (explainer-podcast axis) + #588 (storytelling axis) + #589 (corporate-training final axis). `V2_UPGRADED_SUB_GENRES` gate table carries all 29 populated sub-genres. **Five axes fully closed**: exam-prep 6/6 + skill-mastery 5/5 + explainer-podcast 6/6 + storytelling 7/7 + corporate-training 5/5. Each sub-genre has 2 anchor-diverse alternate exemplars with explicit clarifier coverage; `describe.each(V2_UPGRADED_SUB_GENRES)` gate asserts length bounds, no-prefix overlap, clarifier coverage, and version>=2 for every entry.
...
- **2026-04-24 frontend #576** — sub-genre batch 4 (explainer-podcast axis close): `explainer-podcast/health` (ZOE Science & Nutrition SELECT-trial / Freakonomics MD NHS-Galleri), `explainer-podcast/history` (Dan Carlin Catilinarian crisis / Mike Duncan Shays' Rebellion), `explainer-podcast/technology` (Oxide and Friends io_uring attack surface / Security-Cryptography-Whatever post-quantum TLS). **Explainer-podcast sub-genre axis now 6/6 v2 complete.** Clarifier schemas per sub-genre now documented (health: `evidence_tier_strength` + `population_caveat`; history: `dated_historical_window` + `primary_source_lineage`; technology: `concrete_artifact_to_walk_through` + `tradeoff_the_winner_pays`).
- **2026-04-24 frontend #588** — sub-genre batch 5 (storytelling axis close): `storytelling/mystery` (Chandler/Hammett hard-boiled LA noir 1947 + Mankell/Läckberg Nordic procedural Jämtland Feb 2019), `storytelling/sci-fi` (Peter Watts Blindsight cold-hard first-contact 2047 + Becky Chambers Wayfarers solarpunk Pachamama-3 L5 2071). **Storytelling sub-genre axis now 7/7 v2.** Genre-specific 8th clarifier keys: mystery → `fair_play_clue_discipline`; sci-fi → `rigor_constraint`.
- **2026-04-24 frontend #589** — sub-genre batch 6 (**corporate-training final axis close**): `corporate-training/data-privacy` (Change Healthcare Feb 2024 / Schrems II EU→US transfers), `corporate-training/dei` (Epic Games FTC 2022 COPPA / Trusaic SSEG pay-equity 2025), `corporate-training/manager-onboarding` (Fournier *Manager's Path* M1 new-manager / GitLab async-first L6 staff lift), `corporate-training/sales-enablement` (Dixon/Adamson *Challenger Sale* / Winning by Design SPICED mid-market). **Corporate-training sub-genre axis now 5/5 v2 complete.** 7-clarifier shape matches shipped `corporate-training/cybersecurity`: regulation/framework + article, audience L-level + headcount, LMS target, mastery gate %, named anchor incident/case, specific scenario, cert deadline.
- **2026-04-24 — full scenario-guidance v2 data-layer sweep complete at 29/29** across all five sub-genre axes (exam-prep 6/6 + skill-mastery 5/5 + explainer-podcast 6/6 + storytelling 7/7 + corporate-training 5/5). `V2_UPGRADED_SUB_GENRES` gate table carries 198 describe.each v2 assertions; frontend jest 1017/1017 pass.
