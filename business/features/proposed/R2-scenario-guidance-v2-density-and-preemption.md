...
- [ ] All 29 sub-genre entries rewritten under the same contract. *(25 registered in `V2_UPGRADED_SUB_GENRES` gate table through 2026-04-24 — 4 remaining: `corporate-training/data-privacy`, `corporate-training/dei`, `corporate-training/manager-onboarding`, `corporate-training/sales-enablement`. **exam-prep 6/6** + **skill-mastery 5/5** + **explainer-podcast 6/6** + **storytelling 7/7** axes fully closed.)*
...
- *(next)* sub-genre v2 sweep — **4 entries remaining, all in corporate-training**: data-privacy, dei, manager-onboarding, sales-enablement. Mirror the shipped `corporate-training/cybersecurity` 7-clarifier shape (regulation/framework + article, audience L-level, headcount, LMS target, mastery gate %, named anchor incident, cert deadline).
