...
- [x] RTL CSS foundations in place — frontend PR #493: `lib/rtl.ts` resolves BCP 47 → direction via primary-subtag match (covers ar/he/fa/ur/ps/sd/ckb/yi/dv + legacy `iw`); `components/LanguageDirection.tsx` sets `document.documentElement.dir` from the signed-in user's R3 Phase 1 `language_preference`; `app/layout.tsx` SSR default `dir="ltr"`. Full Tailwind ml-/mr- → ms-/me- sweep deliberately deferred until real Arabic/Hebrew audio content ships.
