...
| Picker routes to the wrong episode_type (e.g. user picks "drill" but really wants "pressure") | When the picker sets `preferences["_episode_type"]`, the classifier does **NOT** run at all per #353's `_resolve_episode_type` short-circuit (explicit preference returns before the classifier is invoked). So mis-picks are the "wizard wasn't specific enough" failure, not a classifier mis-route. Mitigation: iterate the 5-question wizard post-ship rather than adding classifier-override logic (the short-circuit is intentional and load-bearing — it preserves explicit user intent against ambiguous title phrases). |
...
4. **Classifier + picker interaction** (clarified post-audit-of-own-proposal): per #353's `_resolve_episode_type`, an explicit picker selection SHORT-CIRCUITS the classifier — it doesn't run at all when `preferences["_episode_type"]` is already set. This is intentional and correct behavior. The open question is: if a user types "Night before your interview" in the topic WITHOUT clicking a card, should we (a) silently route to final_prep via the classifier (current behavior when `ENABLE_EPISODE_TYPE_CLASSIFIER=true`), (b) surface a soft prompt ("Looks like you want final-prep mode — want to select it?"), or (c) force a picker click for interview-prep traffic? Options (b) and (c) are UX decisions; the workers-side behavior is fixed.
