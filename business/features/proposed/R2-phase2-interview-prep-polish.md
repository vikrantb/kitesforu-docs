...
- [x] Gaps link to specific episodes that address them — kitesforu-api PR #253 rebases the stale curriculum-builder endpoint onto current main (`POST /v1/interview-prep/build-curriculum`). Each returned `DraftEpisode` carries a `gap_id` linking it back to the confirmed gap it addresses. Unblocks the frontend's silent 404 on `handleBuildCurriculum`. Also added explicit timeout + 1 retry matching the reliability pattern from PR #251. Live on api rev 00497.
