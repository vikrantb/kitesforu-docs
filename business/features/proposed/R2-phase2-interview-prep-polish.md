...
- [ ] Gaps link to specific episodes that address them — **INFRASTRUCTURE SHIPPED, USER-FACING RENDER STILL MISSING** (self-correction 2026-04-21). kitesforu-api PR #253 rebased the curriculum-builder endpoint so each returned `DraftEpisode` carries a `gap_id` linking it to the confirmed gap. But the `gap_id` → episode mapping is NOT currently surfaced to users anywhere: the course-detail `<GapAnalysisCard>` (frontend PR #492/#494) renders gaps + strengths but doesn't show which episodes address each gap, and the draft episodes from the curriculum-builder endpoint aren't persisted on the course doc — only the gaps + strengths blob is. To actually close this AC: (a) persist the draft episodes with their gap_ids on the course doc at create time, AND (b) render a "Covered in: Episode 3, Episode 5" line under each gap in `<GapAnalysisCard>`. Follow-up work; NOT shipped.
